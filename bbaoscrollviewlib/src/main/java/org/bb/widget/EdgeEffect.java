...
public class EdgeEffect {
	 ...
	 * 
	 * @param context
	 *            Context used to provide theming and resource information for
	 *            the EdgeEffect
	 */
	public EdgeEffect(Context context) {
		final Resources res = context.getResources();
		mEdge = res.getDrawable(getPlatformDrawableId("overscroll_edge"));
		mGlow = res.getDrawable(getPlatformDrawableId("overscroll_glow"));

		mEdgeHeight = mEdge.getIntrinsicHeight();
		mGlowHeight = mGlow.getIntrinsicHeight();
		mGlowWidth = mGlow.getIntrinsicWidth();

		mMaxEffectHeight = (int) (Math.min(mGlowHeight * MAX_GLOW_HEIGHT
				* mGlowHeight / mGlowWidth * 0.6f, mGlowHeight
				* MAX_GLOW_HEIGHT) + 0.5f);
	...
	private int getPlatformDrawableId(String name) {
		try {
			int i = ((Integer) Class.forName("com.android.internal.R$drawable")
	...
	/**
	 * Set the position of this edge effect in pixels. This position is only
	 * used by {@link #getBounds(boolean)}.
	 * 
	 * @param x
	 ...
	 * 
	 * @param deltaDistance
	 *            Change in distance since the last call. Values may be 0 (no
	 *            change) to 1.f (full length of the view) or negative values to
	 *            express change back toward the edge reached to initiate the
	 *            effect.
	 */
	...
	public boolean draw(Canvas canvas) {
		...
		int glowBottom = (int) Math.min(mGlowHeight * mGlowScaleY * mGlowHeight
				/ mGlowWidth * 0.6f, mGlowHeight * MAX_GLOW_HEIGHT);
		if (mWidth < mMinWidth) {
