...
public class OverScroller {
	...
	public boolean springBack(int startX, int startY, int minX, int maxX, int minY, int maxY) {
		...
		Log.e("overscroller", "springBack--- -startY = " + startY + " , minY = " + minY + ", maxY = " + maxY);
		Log.e("overscroller", "springBack--- -startX = " + startX + " , minX = " + minX + ", maxX = " + maxX);
	...
	static class SplineOverScroller {
		...
		private static final int FAST_CUBIC = 3;
		...
		void finish() {
			...
			// TODO: restore when WebView uses the fast fling implemented in
			// this class.
			// mCurrVelocity = 0.0f;
			mFinished = true;
		}
...
//			mState = BALLISTIC;
			...
			mDuration = (int) (1000.0 * Math.sqrt(-0.333 * delta / mDeceleration));
		}
		...
		private void startAfterEdge(int start, int min, int max, int velocity) {
			...
			if (keepIncreasing) {
				// Will result in a bounce or a to_boundary depending on
				// velocity.
		...
		void notifyEdgeReached(int start, int end, int over) {
			...
			if (mState == SPLINE) {
				...
				// that distance to
				// edge is increasing. This ensures that startAfterEdge will not
				// start a new fling.
		...
		private void onEdgeReached() {
			...
			if (distance > mOver) {
				// Default deceleration is not sufficient to slow us down before
				// boundary
